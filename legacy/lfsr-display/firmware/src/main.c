...
__fuse_t FUSEMEM fuse = {.low = LFUSE_DEFAULT, .high = HFUSE_DEFAULT};

Seed EEMEM seed = {.s0 = 6378};
...
static void timer_compare_callback() {
...
static void timer_ovf_callback() {
	TIFR0 = (1 << TOV0); // shouldn't be needed according to the documentation, but is (perhaps the ISR is too short
						 // without it?)
...
ISR(TIM0_OVF_vect) { timer_ovf_callback(); }

static void pin_down_callback() {

	/*	Both pins high implies no button is pressed.	*/
	if ((PINB & seedPin) == seedPin || (PINB & nextPin) == nextPin) {
		/*	Disable pin.	*/
		TIMSK0 &= ~(1 << OCIE0A);
	}
...
ISR(PCINT0_vect) { pin_down_callback(); }
